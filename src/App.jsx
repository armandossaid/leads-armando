...
const STEPS = [
  { id: "nombre", question: "¿Cómo te llamas?", subtitle: "Para saber a quién le escribo 👋", type: "text", placeholder: "Tu nombre completo", key: "nombre" },
  { id: "whatsapp", question: "¿Cuál es tu WhatsApp?", subtitle: "Te contacto directamente por ahí", type: "tel", placeholder: "+52 55 1234 5678", key: "whatsapp" },
  { id: "email", question: "¿Y tu correo?", subtitle: "Para enviarte información detallada", type: "email", placeholder: "tu@correo.com", key: "email" },
  { id: "interes", question: "¿Qué estás buscando?", subtitle: "Puedes elegir más de una opción", type: "multi", key: "interes", options: [{ value: "comprar_vivir", label: "🏠 Comprar para vivir" }, { value: "comprar_invertir", label: "📈 Comprar para invertir" }, { value: "preventa", label: "🏗️ Preventa" }, { value: "rentar", label: "🔑 Rentar" }, { value: "vender", label: "💼 Vender mi propiedad" }] },
  { id: "zona", question: "¿En qué zona o ciudad te interesa?", subtitle: "Ciudad, colonia o estado — lo que tengas en mente", type: "text", placeholder: "Ej: Roma Norte, CDMX / Puerto Vallarta / Mérida", key: "zona" },
  { id: "presupuesto", question: "¿Con qué presupuesto cuentas?", subtitle: "Aproximado está bien, sin compromiso", type: "single", key: "presupuesto", options: [{ value: "menos_2", label: "Menos de $2 MDP" }, { value: "2_5", label: "$2 – $5 MDP" }, { value: "5_10", label: "$5 – $10 MDP" }, { value: "mas_10", label: "Más de $10 MDP" }, { value: "no_definido", label: "Aún no lo defino" }] },
  { id: "timeline", question: "¿En qué tiempo piensas tomar una decisión?", subtitle: "Así sé qué tan urgente es tu búsqueda", type: "single", key: "timeline", options: [{ value: "ya", label: "⚡ Ya estoy listo" }, { value: "3meses", label: "📅 En los próximos 3 meses" }, { value: "6meses", label: "🗓️ En 6 meses" }, { value: "explorando", label: "👀 Solo estoy explorando" }] },
  { id: "video", question: "¿Algún video te llamó la atención?", subtitle: "Opcional — si recuerdas cuál fue, cuéntame brevemente", type: "textarea", placeholder: "Ej: el video del depto en Polanco, el de inversión en Mérida...", key: "video", optional: true },
  { id: "comentarios", question: "¿Algo más que quieras contarme?", subtitle: "Opcional — pero entre más detalles, mejor te ayudo", type: "textarea", placeholder: "Cuéntame lo que necesites...", key: "comentarios", optional: true },
];

const SCORE_MAP = { timeline: { ya: 25, "3meses": 20, "6meses": 10, explorando: 0 }, presupuesto: { menos_2: 5, "2_5": 10, "5_10": 15, mas_10: 20, no_definido: 0 }, interes: { comprar_invertir: 15, preventa: 12, comprar_vivir: 10, vender: 8, rentar: 5 } };
...
function calcScore(data) {
  ...
  if (data.interes?.length) score += Math.max(...data.interes.map((i) => SCORE_MAP.interes[i] || 0));
...
function ScoreTag({ score }) {
  ...
  return <span style={{ background: color + "22", color, border: `1px solid ${color}44`, borderRadius: 999, padding: "2px 12px", fontSize: 12, fontWeight: 600 }}>{label} · {score} pts</span>;
}
...
export default function LeadForm() {
  ...
  function animateTransition(dir, cb) { setAnimDir(dir); setVisible(false); setTimeout(() => { cb(); setVisible(true); }, 200); }
  function canProceed() { if (current.optional) return true; if (current.type === "multi") return multiVal.length > 0; if (current.type === "single") return !!data[current.key]; return inputVal.trim().length > 0; }
  ...
  function handleNext() {
    ...
    setData(newData);
    if (step < STEPS.length - 1) { animateTransition("forward", () => setStep((s) => s + 1)); }
    else {
      ...
      fetch("https://script.google.com/macros/s/AKfycbzHyef8vBLo8VhNBEcduN054TEqAdvOO6fx_WByjl5CywbZBUf5Ni5GQyN-A_E2beuL/exec", { method: "POST", mode: "no-cors", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...lead, interes: Array.isArray(lead.interes) ? lead.interes.join(", ") : lead.interes }) }).catch(console.error);
      setDone(true);
    }
  }
      ...
      <div style={{ minHeight: "100vh", background: "#0a0a0f", fontFamily: "system-ui, sans-serif", padding: "40px 24px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
            <div><div style={{ color: "#c9a96e", fontSize: 12, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 4 }}>Armando Said</div><div style={{ color: "#fff", fontSize: 24, fontWeight: 700 }}>Panel de Leads</div></div>
          ...
          {leads.length === 0 ? <div style={{ textAlign: "center", color: "#333", padding: "80px 0" }}>Aún no hay leads.</div> : (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {leads.map((lead, i) => (
                <div key={i} style={{ background: "#13131a", border: "1px solid #222", borderRadius: 16, padding: 24 }}>
                  ...
                  {lead.video && <div style={{ marginTop: 12, background: "#0a0a0f", borderRadius: 10, padding: "10px 14px" }}><div style={{ color: "#555", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 3 }}>Video</div><div style={{ color: "#ccc", fontSize: 14 }}>{lead.video}</div></div>}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", display: "flex", flexDirection: "column", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <div style={{ height: 2, background: "#111" }}><div style={{ height: "100%", background: "linear-gradient(90deg, #c9a96e, #e8c98a)", width: `${progress}%`, transition: "width 0.4s ease", borderRadius: 2 }} /></div>
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 24px" }}>
        <div style={{ maxWidth: 520, width: "100%", opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : animDir === "forward" ? "translateY(16px)" : "translateY(-16px)", transition: "opacity 0.2s ease, transform 0.2s ease" }}>
          ...
          <div style={{ animation: shake ? "shake 0.4s ease" : "none" }}>
            ...
            {current.type === "single" && <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{current.options.map((opt) => <button key={opt.value} onClick={() => handleSingle(opt.value)} style={{ background: data[current.key] === opt.value ? "rgba(201,169,110,0.12)" : "rgba(255,255,255,0.04)", border: `1px solid ${data[current.key] === opt.value ? "#c9a96e" : "rgba(255,255,255,0.08)"}`, borderRadius: 12, color: data[current.key] === opt.value ? "#c9a96e" : "#aaa", padding: "14px 18px", textAlign: "left", fontSize: 15, cursor: "pointer", transition: "all 0.15s", fontFamily: "inherit", fontWeight: data[current.key] === opt.value ? 600 : 400 }}>{opt.label}</button>)}</div>}
            {current.type === "multi" && <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{current.options.map((opt) => { const sel = multiVal.includes(opt.value); return <button key={opt.value} onClick={() => handleMulti(opt.value)} style={{ background: sel ? "rgba(201,169,110,0.12)" : "rgba(255,255,255,0.04)", border: `1px solid ${sel ? "#c9a96e" : "rgba(255,255,255,0.08)"}`, borderRadius: 12, color: sel ? "#c9a96e" : "#aaa", padding: "14px 18px", textAlign: "left", fontSize: 15, cursor: "pointer", transition: "all 0.15s", fontFamily: "inherit", fontWeight: sel ? 600 : 400, display: "flex", alignItems: "center", gap: 10 }}><span style={{ width: 18, height: 18, borderRadius: 4, border: `1.5px solid ${sel ? "#c9a96e" : "#444"}`, background: sel ? "#c9a96e" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{sel && <span style={{ color: "#0a0a0f", fontSize: 12, fontWeight: 900 }}>✓</span>}</span>{opt.label}</button>; })}</div>}
          </div>
